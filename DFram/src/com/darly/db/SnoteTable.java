...
import com.darly.common.LogApp;
import com.darly.model.Card_Shop_List_Cart;
import com.darly.model.SNote;
...
public class SnoteTable extends BaseTable<SNote> implements SnoteDAO {
	...
	public List<Card_Shop_List_Cart> getChange() {
		List<Card_Shop_List_Cart> list = new ArrayList<Card_Shop_List_Cart>();
		Cursor c = db.query(TABLE_NAME, null, null, null, null, null, null);
		while (c.moveToNext()) {
			int product_id = c.getInt(_PRODUCT_ID);
			int product_num = c.getInt(_PRODUCT_NUM);
			...
			Card_Shop_List_Cart cart = new Card_Shop_List_Cart(null, null, 0,
					product_id, product_num, product_name, product_url,
					product_price, product_orprice, product_spec, null,
					product_image, product_reless);
