...
public class ImageGalleryFragment extends CCPFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mEntry = getArguments() != null ? getArguments()
				.<ViewImageInfo> getParcelable("entry") : null;
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		mImageView.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				ECListDialog dialog = new ECListDialog(getActivity(),
						new String[] { getString(R.string.save_to_local) });
		...
		ImageLoader.getInstance().displayImage(mImageUrl, mImageView,
				imageOptionsBuilder.build(), new SimpleImageLoadingListener() {
					@Override
					...
					public void onLoadingComplete(String imageUri, View view,
						...
						progressBar.setVisibility(View.GONE);
						imgCacheFile = DiskCacheUtils.findInCache(imageUri,
								ImageLoader.getInstance().getDiskCache());
						if (imgCacheFile != null) {
							if (loadedImage.getHeight() > DemoUtils
									.getScreenHeight(getActivity())) {
								mCacheImageUrl = "file://"
										+ imgCacheFile.getAbsolutePath();
							...
							if (imageUri.startsWith("http:")) {
								ImgInfo thumbimginfo = ImgInfoSqlManager
										.getInstance().getImgInfo(
												mEntry.getIndex());
								if (thumbimginfo != null
										&& mCacheImageUrl != null) {
									thumbimginfo.setBigImgPath(mCacheImageUrl
											.substring(mCacheImageUrl
													.lastIndexOf("/")));
									ImgInfoSqlManager.getInstance()
											.updateImageInfo(thumbimginfo);
								}
							}
						}
					}
