...
//                checkContact(message.getForm());??
        ...
        try {
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    long id = cursor.getLong(cursor.getColumnIndex(BaseColumn.ID));
    ...
    public static void deleteChattingMessage(String contactId) {
		...
		ConversationSqlManager.delSession(contactId);
    }
    ...
    public static ArrayList<ECMessage> queryIMessageVersionList(long threadId, int num, String version) {
        ...
        if (version != null && !version.equals("") && !version.equals("0")) {
            sb.append(BaseColumn.ID + " < ").append(version);
        } else {
            sb.append("1=1");
        }
        // if (threadId != 0) {
    ...
    public static int deleteMulitMsg(List<Long> rowIds) {
        ...
        StringBuilder where = new StringBuilder(BaseColumn.ID + " IN (");
    ...
    public static int changeResendMsg(long rowid , ECMessage detail) {
        ...
        String where = BaseColumn.ID + "=" + rowid + " and " + IMessageColumn.SEND_STATUS + " = " + ECMessage.MessageStatus.FAILED.ordinal();
