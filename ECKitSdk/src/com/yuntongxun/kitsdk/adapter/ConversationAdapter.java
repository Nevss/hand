...
public class ConversationAdapter extends CCPListAdapter <ECConversation> {
    ...
    public ConversationAdapter(Context ctx , OnListAdapterCallBackListener listener,OnGetConversationInfoListener infoListener) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(conversation != null) {
            ...
            if((conversation.getSessionId().toUpperCase().contains("G")&&conversation.getContent()==null)||conversation.getSessionId().equals("10000"))
            {
            ...
            if(!conversation.getSessionId().equals("10000")){
	            if(mGetPersonInfoListener!=null)
	            	mGetPersonInfoListener.getConversationInfo(conversation,mViewHolder.nickname_tv,mViewHolder.user_avatar);
            } else {
            	mViewHolder.last_msg_tv.setText("");
            	mViewHolder.update_time_tv.setText("");
            	 if(mGetPersonInfoListener!=null){
 	            	mGetPersonInfoListener.getBMYXZSConversationInfo(conversation,mViewHolder.last_msg_tv,mViewHolder.update_time_tv);
            	 }
