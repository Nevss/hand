...
public class GroupChatActivity extends GroupBaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		LogUtil.i("wpc2", "GroupChatActivity===false");
		...
		try {
			...
			chatInfoBean = dbUtils.findFirst(Selector.from(ChatInfoBean.class)
					.where("docInfoBeanId", "=", mGroupId));

			if (chatInfoBean != null
					&& SharedPrefsUtil.getValue(
							"RM_" + chatInfoBean.getSubjectID(), false) == false) {
	...
	private void initView() {
			...
			public void onClick(View v) {
				...
				if (isComment) {
					if (DBUtilsHelper.getInstance().isOnline()) {
						...
						reConsult_bt.setClickable(false);
						requestChatAgain();
					}
				} else {
					Intent i = new Intent();
		...
		TimerService.setmHandler(new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				// || CCPHelper.getInstance().getDevice().isOnline() !=
				// State.ONLINE) {
				...
				if (msg.arg1 >= Constants.PROGRESSMAX) {
					...
					if (chatInfoBean2 != null) {
						SharedPrefsUtil.putValue(
								"RM_" + chatInfoBean2.getSubjectID(), true);
					}
					try {
					...
					// HttpClient.get(GroupChatActivity.this,
					// Constants.CLOSESUBJECT_URL,
	...
	private void initialize(Bundle savedInstanceState) {
		...
		if (intent.hasExtra("groupId")) {
			...
			gc_t_re.getLayoutParams().height = (int) TypedValue.applyDimension(
			...
			try {
				...
				if (null != chatInfoBean) {
					...
					if (docInfoBean.getVoipAccount().equals(docinfoId)) {
						...
						myProgressbar_re.setVisibility(View.VISIBLE);
						gc_t_re.getLayoutParams().height = (int) TypedValue
	...
	public void onRecordStart() {
		...
		mIMChatHandler.sendEmptyMessageDelayed(WHAT_ON_COMPUTATION_TIME,
				CCPBaseActivity.TONE_LENGTH_MS);
	}
	...
	private class MyTask extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... params) {
			...
			Constants.scrote = ImageUtil.readPictureDegree(file
					.getAbsolutePath());
			...
			try {
				...
				if (fileInputStream.available() / 1024 > 300) {
					...
					// .getAbsolutePath());
					Bitmap bitmap = BitmapCompressorUtil
							.decodeSampledBitmapFromFile(
									takepicfile.getAbsolutePath(), 1200, 1600);
	...
	class IMListyncTask extends
		...
		protected void onPostExecute(ArrayList<IMChatMessageDetail> result) {
			...
			if (result != null && !result.isEmpty()) {
				...
				mIMGroupApapter = new IMGroupChatItemAdapter(result);
				mIMGroupListView.setAdapter(mIMGroupApapter);
				mIMGroupListView.setSelection(mIMGroupApapter.getCount());
			} else {
				if (result == null)
	...
	protected void onReceiveBroadcast(Intent intent) {
		...
		if (CCPIntentUtils.INTENT_IM_RECIVE.equals(intent.getAction())
			...
			if (intent.hasExtra(KEY_GROUP_ID)) {
				...
				if (docInfoBean != null
						&& !intent.getStringExtra(KEY_GROUP_ID).equals(
								docInfoBean.getVoipAccount())
						&& !intent.hasExtra(KEY_TRANSFORM)) {
				...
				// listView.
				if (sender.equals(mGroupId) && !intent.hasExtra(KEY_MESSAGE_ID)
						&& chatModel != CHAT_MODE_IM_GROUP
						&& !intent.hasExtra(KEY_IS_LOCAL)) {
					gc_title2.setVisibility(View.GONE);
					SharedPrefsUtil.putValue(
							"RM_" + chatInfoBean2.getSubjectID(), true);
				}
				...
				if (!TextUtils.isEmpty(sender) && !sender.equals(mGroupId)) {
					try {
						...
						if (null == docInfoBean) {
							...
							// onSendTextMesage("您已成功转诊至" +
							// docInfoBean.getUserName() +
							// "医生",UserDataUtils.getUserDataForTrans(subjectID,doctorOld));
							// if(chatInfoBean2.getSubjectType().equals("3")){
							// String content =
							// SharedPrefsUtil.getValue("msg_content", "");
		...
		HttpClient.delete(GroupChatActivity.this, Constants.CHATAGAIN_URL,
				requestParams, new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						...
						reConsult_bt.setClickable(true);
					}
		...
		try {
			...
			if (!subjectID.equals("-1")) {
				...
				try {
					...
					chatInfoBean2 = chatInfoBean;
					SharedPrefsUtil.putValue(
							"RM_" + chatInfoBean2.getSubjectID(), false);
					gc_title2.setVisibility(View.VISIBLE);
				} catch (DbException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
