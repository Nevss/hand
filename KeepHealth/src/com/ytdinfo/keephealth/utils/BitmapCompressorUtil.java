...
public class BitmapCompressorUtil {
	...
	public static Bitmap compressBitmap(Bitmap image, int maxkb) {
		...
		ImageTools.savePhotoToSDCard(baos, Constants.IMAGES_DIR,
	...
	@SuppressLint("NewApi")
	public static String compressBitmap2(Bitmap image, int maxkb,
		...
		while (options > 0) {
			...
			// bitmap.recycle();
			// Log.i("paul","压缩一次!---" +options+"==="+baos.toByteArray().length
			// / 1024);
		}
		// FileInputStream
		try {
		...
		if (Constants.scrote == 0) {
			return smallFileName;
		}
		Bitmap rotated_bm = ImageUtil.rotateBitmapByDegree(
				BitmapFactory.decodeFile(smallFileName), Constants.scrote);
		String name = System.currentTimeMillis() + "";
		ImageTools.savePhotoToSDCard(rotated_bm, Constants.IMAGES_DIR, name);
		// 使用过后。将角度重置为零。
		Constants.scrote = 0;
		return Constants.IMAGES_DIR + name + ".png";
		// Log.i(test,压缩后大小 + baos.toByteArray().length);
