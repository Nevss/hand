...
public class MyPopWindow {
	...
	public void initPop() {
		...
		bt1.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				((Activity) mContext).startActivityForResult(openCameraIntent, Constants.CAMERA_WITH_DATA);
			}
		});
	...
	/*public Bitmap INonActivityResult(int requestCode, Intent data) {
		if (data == null) {
			return null;
		}
		...
		case Constants.CAMERA_WITH_DATA: {
			...
			uri_temp_crop_picture = Uri
					.fromFile(new File(Constants.IMAGES_DIR));
		...
		case Constants.PHOTO_PICKED_WITH_DATA: {
			...
			try {
				...
				Bitmap tempBitmap2 = MediaStore.Images.Media.getBitmap(
						resolver, uri_temp_crop_picture);
		...
		case Constants.CROP_PICKED_WITH_DATA: {
			LogUtil.i(TAG, "裁剪");
			if (data != null) {
		 ...
		 * != null) { //为防止原始图片过大导致内存溢出，这里先缩小原图显示，然后释放原始Bitmap占用的内存 Bitmap
		 * smallBitmap = ImageTools.zoomBitmap(photo, photo.getWidth() / SCALE,
		 * photo.getHeight() / SCALE); //释放原始图片占用的内存，防止out of memory异常发生
		 * photo.recycle();
		 * 
		 * // iv_image.setImageBitmap(smallBitmap); } } catch
		 * (FileNotFoundException e) { e.printStackTrace(); } catch (IOException
		 * e) { e.printStackTrace(); } break;
		 * 
		 * 
		 * case CROP: Uri uri = null; if (data != null) { uri = data.getData();
		 * System.out.println("Data"); }else { System.out.println("File");
		 ...
		 * 
		 * case CROP_PICTURE: Bitmap photo = null; Uri photoUri =
		 * data.getData(); if (photoUri != null) { photo =
		 ...
		 * 100, stream); } } //iv_image.setImageBitmap(photo); break;
