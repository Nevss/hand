...
public class MediaPlayTools {
	...
	private void play(boolean isEarpiece , int seek) {
		...
		try {
			...
			e.printStackTrace();
			Log4Util.v(TAG, "[MediaPlayTools - play ] playImp : fail, exception = " + e.getMessage());
		}
	}
	...
	* @see #play(boolean, int)
	...
	private boolean play(String urlPath , boolean isEarpiece , int seek) {
		...
		if(status != STATUS_STOP) {
			Log4Util.e(TAG, "[MediaPlayTools - play ] startPlay error status:" + status);
			return false;
		}
		...
		try {
			...
			try {
				...
				e1.printStackTrace();
				result = false;
				Log4Util.v(TAG, "[MediaPlayTools - play ] startPlay File[" + this.urlPath + "] failed");
			}
	...
	* @return
	...
	public boolean resume() {
		...
		if( this.status != STATUS_PAUSE) {
			...
			Log4Util.e(TAG, "[MediaPlayTools - resume ] resume not STATUS_PAUSE error status:" + this.status);
			return false;
		}
		...
		try {
			...
			e.printStackTrace();
			Log4Util.e(TAG, "[MediaPlayTools - resume ] resume File[" + this.urlPath + "] ErrMsg[" + e.getStackTrace() + "]");
	...
	* @return
	...
	public boolean stop () {
		...
		try {
			...
			e.printStackTrace();
			Log4Util.v(TAG, "[MediaPlayTools - stop]stop File[" + this.urlPath + "] ErrMsg[" + e.getStackTrace() + "]");
			this.status = STATUS_ERROR;
			result = false;
	...
	* @param speakerOn
	...
	public void setSpeakerOn(boolean speakerOn) {
		...
		Log4Util.v(TAG, "[MediaPlayTools - setSpeakerOn] setSpeakerOn=" + speakerOn);
	...
	public boolean pause () {
		if(this.status != STATUS_PLAYING) {
			Log4Util.e(TAG, "[MediaPlayTools - pause]pause not STATUS_PLAYING error status:" + this.status);
			return false;
		}
		...
		try {
			...
			e.printStackTrace();
			Log4Util.v(TAG, "[MediaPlayTools - pause] pause File[" + this.urlPath + "] ErrMsg[" + e.getStackTrace() + "]");
			result = false;
			status = STATUS_ERROR;
		}
	...
	private void setOnCompletionListener() {
		...
		mediaPlayer.setOnCompletionListener(new OnCompletionListener() {
			...
			public void onCompletion(MediaPlayer mp) {
				Log4Util.d(TAG, "[MediaPlayTools - setOnCompletionListener] Play file[" + urlPath+ "] com");
				status = STATUS_STOP;
			}
		});
	}
