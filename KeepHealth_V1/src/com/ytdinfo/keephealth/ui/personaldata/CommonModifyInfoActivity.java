...
/**
 * @author Zhangyuhui
 * CommonModifyInfoActivity
 * $PersonalDataActivity进行资料修改调用本类。
 * 下午5:44:09
 * TODO本来用来修改用户姓名。用户身份证号。用户手机号。
 */
...
//					if(et.getText().toString().length()!=18){
					if(!IDCardUtils.IDCardValidate(et.getText().toString().trim()).equals("")){
					...
					new RequestCallBack<String>() {
						...
						public void onSuccess(ResponseInfo<String> responseInfo) {
							...
							if (title.equals("姓名")) {
								...
								// 存入年龄
								userModel.setAge(MathUtils.calculateAge(et.getText().toString()
										.trim()));
