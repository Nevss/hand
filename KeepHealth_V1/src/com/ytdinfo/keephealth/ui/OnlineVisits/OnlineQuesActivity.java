...
//					startActivityForResult(intent, 0x123);
	...
	public void saveMe() {
		...
		if (et_age.getText().toString().trim().equals("")) {
			...
			model.setAge(Integer.parseInt(et_age.getText().toString().trim()));
		}
		LogUtil.i(TAG, model.toString());
		...
		case R.id.id_bt_save:
			...
			if(ll.getVisibility()==View.VISIBLE){
				...
				if(et_age.getText().toString().equals("")){
					...
					onlineQuestionUserModel.setAge(Integer.parseInt(et_age.getText().toString()));
				}
	...
	private void goIntent(ChatInfoBean chatInfoBean) {
		...
		Intent i = new Intent();
		i.setClass(OnlineQuesActivity.this, GroupChatActivity.class);
		Bundle bundle = new Bundle();
		bundle.putSerializable("chatInfoBean", chatInfoBean);
		bundle.putStringArrayList("imageList",
				(ArrayList<String>) list_imagesUrl);
		bundle.putString("content", bodyContenet);
		i.putExtras(bundle);
		startActivity(i);
	}
