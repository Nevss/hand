...
public class RegisterActivity extends BaseActivity implements OnClickListener {
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			if(msg.what==0x321){
				HandlerUtils.listenET(new EditText[]{et_telephone,et_yanzhengma}, bt_register.bt_common);
			}
		};
	};
	...
	private void resetTimer() {
		timer.cancel();
		time_now = 61;
		ibt_yanzhengma.setClickable(true);
...
//			requestRegister();
	...
	private void requestVerificationCodeOK() {
		try {
			...
			jsonParam.put("Mobilephone", et_telephone.getText().toString().trim());
	...
	private void requestVerificationCode() {
		try {
			...
			jsonParam.put("UserAccount", et_telephone.getText().toString()
					.trim());
			HttpClient.post(Constants.VERIFICATION_URl, jsonParam.toString(), new RequestCallBack<String>() {
						@Override
						public void onStart() {
						}
...
//				ibt_yanzhengma.setClickable(false);
