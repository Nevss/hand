...
public class EmoticonUtil {
	...
	public static void initEmoji() {
		...
		String[] stringArray = MyApp.getInstance()
	...
	public static int getEmojiResid(int num) {
		return getEmoticonResId(num + "", MyApp.getInstance().getApplicationContext());
	}
	...
	public void initEmojiIcons() {
		Class<?> c = com.ytdinfo.keephealth.R.drawable.class;
		...
		while(i < charArray.length) {
			...
			if(emojiId != -1) {
				...
				if(drawable != null) {
					...
					spannableString.setSpan(new ImageSpan(drawable, 0), i, i + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
