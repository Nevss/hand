...
//	}
	...
	private StringBuffer getServerUrl(String server) {
		// 
		if(!MyApp.getInstance().isDeveloperMode()&&"app.cloopen.com".equals(server)) {
	...
	protected void handleRequestFailed(int requestType ,final String errorCode , final String errorMessage) {
		...
		if(mRequestHandler != null) {
			mRequestHandler.post(new Runnable() {
				...
				@Override
				public void run() {
					Toast.makeText(MyApp.getInstance().getApplicationContext(), errorMessage + "[" +errorCode+ "]", Toast.LENGTH_LONG).show();
				}
			});
		}
	}
