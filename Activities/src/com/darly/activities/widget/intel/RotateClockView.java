...
/**
 * @author Zhangyuhui Clock 下午1:56:03 TODO钟表指示圈。
 */
public class RotateClockView extends View {
	...
	public RotateClockView(Context context) {
		...
		mCircleLineWidth = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, 8, getResources()
						.getDisplayMetrics());
		mHalfCircleLineWidth = mCircleLineWidth;
		mLineWidth = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, 4, getResources()
						.getDisplayMetrics());
		mHalfLineWidth = mLineWidth / 2;

		mFixLineHeight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, 4, getResources()
						.getDisplayMetrics());

		mLongLineHeight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, LONG_LINE_HEIGHT, getResources()
						.getDisplayMetrics());
		mShortLineHeight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, SHORT_LINE_HEIGHT, getResources()
						.getDisplayMetrics());
		initPaint();
	}
	...
	private void drawLines(Canvas canvas) {
		for (int i = 0; i <= 360; i++) {
			...
			if (i % 6 == 0) {
				...
				canvas.drawLine(mLineLeft, mLineTop, mLineLeft, mLineBottom,
						mLinePaint);
				canvas.restore();
			}
		}
	...
	private void drawCircle(Canvas canvas) {
		canvas.drawCircle(mHalfWidth, mHalfHeight, mHalfWidth
				- mHalfCircleLineWidth, mCirclePaint);
	}
