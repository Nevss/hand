...
public class UserInfoPage extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.add_friend:
			...
			if (user.getName().equals(userLogin.getName())) {
				...
				loading.setMessage("正在加为好友...");
				loading.show();
				api.reqAddFriend(user);
			}
			break;
		case R.id.del_frieng:
			loading.setMessage("正在删除好友");
			loading.show();
			api.reqRemoveFriend(user);
			break;
		case R.id.to_bleak:
			loading.setMessage("正在把" + user.getName() + "加入到黑名单");
			loading.show();
			api.reqAddBlocked(user);
			break;
		case R.id.remove_from_black:
			loading.setMessage("正在把" + user.getName() + "移除黑名单");
			loading.show();
	...
	private GotyeDelegate mdelegate = new GotyeDelegate() {
		...
		public void onAddBlocked(int code, GotyeUser user) {
			if (code == 0) {
				UserInfoPage.this.user = user;
				ToastApp.showToast(getBaseContext(), "成功把" + user.getName()
						+ "加入黑名单");
				loading.dismiss();
				initView();
			} else {
				ToastApp.showToast(getBaseContext(), "抱歉，没能把" + user.getName()
						+ "加入黑名单");
				loading.dismiss();
			}
		}
		...
		public void onRemoveFriend(int code, GotyeUser user) {
			...
			if (from == 100) {
				...
				startActivity(i);
				finish();
				ToastApp.showToast(UserInfoPage.this,
						"成功删除好友：" + user.getName());
			} else if (from == 1) {
				finish();
			} else {
		...
		public void onDownloadMedia(int code, GotyeMedia media) {
			if (!TextUtils.isEmpty(media.getUrl())) {
				if (user.getIcon() != null
						&& media.getUrl().equals(user.getIcon().getUrl())) {
